...
export class GraphicsEngine {
    ...
    constructor(canvas_id) {
        ...
        const ret = wasm.graphicsengine_new(ptr0, len0);
        if (ret[2]) {
    ...
    set_rotation(rotation) {
        wasm.graphicsengine_set_rotation(this.__wbg_ptr, rotation);
    }
    /**
     * @param {number} scale
     */
    set_scale(scale) {
        wasm.graphicsengine_set_scale(this.__wbg_ptr, scale);
    }
    /**
     * @param {number} r
     * @param {number} g
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('graphics_engine_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
